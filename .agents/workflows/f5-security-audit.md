...
> **Fase**: F5 — Pruebas / QA
> **Dónde estamos**: Código implementado. Auditamos seguridad antes de desplegar.
> **Por qué esta fase**: Una falla de seguridad en producción expone datos corporativos y puede tener implicaciones legales. Detectar aquí cuesta 10x menos que en producción.
> **Habilita**: Fase 6 (Deploy) — auditoría limpia es requisito para merge a staging.
...
## Auditoría de Lógica AND/OR en Record Rules

Punto de auditoría explícito para reglas de registro:

- **Reglas Globales** (sin grupo): Operan con lógica **AND** — cada regla global RESTRINGE acceso. El usuario debe satisfacer TODAS.
- **Reglas de Grupo** (con `res.groups`): Operan con lógica **OR** — cada regla de grupo EXPANDE acceso. El usuario necesita satisfacer al menos UNA.
- **Superposición**: Las reglas de grupo operan SOLO dentro del perímetro definido por las reglas globales.

### Checklist AND/OR

- [ ] ¿Hay reglas globales? ¿Son demasiado restrictivas (podrían bloquear admin)?
- [ ] ¿Las reglas de grupo expanden acceso correctamente sin romper el perímetro global?
- [ ] ¿Modelos multi-empresa tienen record rule con `['|', ('company_id', '=', False), ('company_id', 'in', company_ids)]`?
- [ ] ¿Usuarios portal tienen reglas dedicadas separadas de las internas?
