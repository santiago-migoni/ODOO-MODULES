...
export function switchColorSchemeItem(env) {
    ...
    return {
        type: "item",
        id: "dipl_web_backend_theme.color_scheme.switch",
        description: markup`
            <div class="d-flex align-items-center justify-content-between gap-3 w-100">
                <span>${_t("Theme")}</span>
                <div class="btn-group">
                    <button class="btn btn-secondary btn-sm ${userColorScheme === "system" ? "active" : ""}" data-color-scheme="system">
                        <span class="visually-hidden">${_t("System")}</span>
                        <i class="fa fa-fw fa-desktop" data-tooltip="${_t("System")}"></i>
                    </button>
                    <button class="btn btn-secondary btn-sm ${userColorScheme === "light" ? "active" : ""}" data-color-scheme="light">
                        <span class="visually-hidden">${_t("Light")}</span>
                        <i class="fa fa-fw fa-sun-o" data-tooltip="${_t("Light")}"></i>
                    </button>
                    <button class="btn btn-secondary btn-sm ${userColorScheme === "dark" ? "active" : ""}" data-color-scheme="dark">
